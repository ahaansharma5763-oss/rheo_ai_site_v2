...
const LEAKS: Leak[] = [
  { icon: 'clock', text: 'An enquiry lands at 7 pm. Your engineer sees it tomorrow morning. Your competitor’s quote went out tonight.' },
  { icon: 'brain', text: 'Your two senior engineers carry the whole catalogue in their heads. Either of them can resign on a Friday.' },
  { icon: 'users', text: 'Dealers wait a day for answers to questions your catalogue already contains, then quote your competitor’s machine instead.' },
  { icon: 'doc',   text: 'A quote takes three days because it waits in a queue behind site visits, meetings, and every other quote.' },
];
      ...
      <div style={{ maxWidth: 'var(--container-max)', margin: '0 auto', position: 'relative', zIndex: 1 }}>
        ...
        <Reveal delay={140}>
          <h2 style={{ fontFamily: 'var(--serif)', fontSize: 'clamp(32px, 4.6vw, 56px)', lineHeight: 1.12, letterSpacing: '-0.02em', color: 'var(--fg)', fontWeight: 500, maxWidth: '22ch', marginBottom: '28px' }}>
            Your revenue has a ceiling. It is the number of engineers you can put on the phone<span style={{ color: 'var(--gold)' }}>.</span>
          </h2>
        </Reveal>
        ...
        <Reveal delay={240}>
          <p style={{ fontFamily: 'var(--sans)', fontSize: 'clamp(15px, 1.3vw, 17px)', lineHeight: 1.75, color: 'var(--fg-mute)', fontWeight: 300, maxWidth: '66ch', marginBottom: 'clamp(48px, 7vh, 72px)' }}>
            You can add products, add dealers, add marketing. But every enquiry still funnels through the same few
            people who actually know the machines. That is the bottleneck, and it leaks money quietly, every day:
          </p>
        </Reveal>
        ...
        <Reveal delay={120}>
          <blockquote style={{ borderLeft: '3px solid var(--gold)', paddingLeft: '28px', maxWidth: '70ch' }}>
            <p style={{ fontFamily: 'var(--serif)', fontSize: 'clamp(19px, 2vw, 26px)', lineHeight: 1.5, color: 'var(--warm-foam)', fontWeight: 500, fontStyle: 'italic' }}>
              You cannot hire your way out of this. Good sales engineers take years to train, cost lakhs a year,
              and walk out with everything they know. The knowledge should belong to the company. Now it can.
            </p>
          </blockquote>
        </Reveal>
      </div>
